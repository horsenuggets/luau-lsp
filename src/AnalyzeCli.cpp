...
static const char* LUNE_GLOBAL_TYPES = R"LUAU(
...
int startAnalyze(const argparse::ArgumentParser& program)
{
    ...
    if (auto platformArg = program.present("--platform"))
    {
        ...
        else if (platformArg == "lune")
            client.configuration.platform.type = LSPPlatformConfig::Lune;
    }
    ...
    if (client.configuration.platform.type == LSPPlatformConfig::Lune)
    {
        auto loadResult = types::registerDefinitions(frontend, frontend.globals, "", std::string(LUNE_GLOBAL_TYPES));
        if (!loadResult.success)
        {
            fprintf(stderr, "Failed to load built-in Lune definitions\n");
            return 1;
        }
    }
